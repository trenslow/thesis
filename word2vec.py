...
def get_clusters(l1, l2, g2p):
    if g2p:
        with open('clusters_g2p.' + l1 + '.' + l2) as cluster_file:
            cluster_lines = islice(cluster_file, 0, None, 2)
            return {cluster.strip()[:-1] for cluster in cluster_lines}
    else:
        with open('clusters.' + l1 + '.' + l2) as cluster_file:
            cluster_lines = islice(cluster_file, 0, None, 2)
            return {cluster.strip()[:-1] for cluster in cluster_lines}
...
def make_translation_map(l2):
    g2p = OD()
    old_graph_len = 0
    with open('training_articles_g2p.' + l2) as l2s:
    ...
    with open('training_articles_g2p.' + l2 + '.err') as l2f:
...
if __name__ == '__main__':
    ...
    vectors = []
    with open('training_articles.' + lang2) as train, open('vectors.' + lang1 + '.' + lang2, 'w+') as vecs:
        for line in train:
            ...
            if phone_rep:
                ...
                max_cs_phone = 0
            vector = [min_ed_graph, min_ed_phone, max_cs_graph, max_cs_phone, graph_rep]
            vecs.write(' '.join(str(el) for el in vector) + '\n')
